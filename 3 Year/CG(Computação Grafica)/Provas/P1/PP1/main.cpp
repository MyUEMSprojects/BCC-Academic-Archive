...
void Poline::drawPoline()
{
   ...
   if(filled) // Se preenchido
   {
      glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); // Define modo de preenchimento
      glBegin(GL_POLYGON); // Começa a desenhar um polígono preenchido
      for(const auto& point : points) // Para cada ponto na lista de pontos
      {
         glVertex3f(point.first, point.second, 0.0); // Desenha o vértice
      }
      glEnd(); // Finaliza o desenho do polígono
   }
   else // Se não preenchido
   {
      glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
// Modifique a função drawTextSelectionBox
void drawTextSelectionBox(void)
{
    ...
    // Draw text icon - "ABC" with different sizes
    glColor3f(0.0, 0.0, 0.0);

    // a
    glRasterPos2f(0.02*width, 0.54*height);
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, 'A');

    // B
    glRasterPos2f(0.045*width, 0.55*height);
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, 'B');

    // c
    glRasterPos2f(0.07*width, 0.53*height);
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, 'C');

    // Draw underline
    glBegin(GL_LINES);
        glVertex2f(0.02*width, 0.52*height);
        glVertex2f(0.08*width, 0.52*height);
    glEnd();
}
...
void drawScene(void)
{
   ...
   else if(primitive == POLYLINE && currentPoline != nullptr)
   {
      ...
      // Mostra uma prévia da linha que vai do último ponto até a posição atual do mouse
      if(currentPoline->getPoints().size() > 0)
      {
         ...
         glBegin(GL_LINES);
         glVertex3f(lastPoint.first, lastPoint.second, 0.0); // Começa no último ponto adicionado
         glVertex3f(currentX, currentY, 0.0); // Termina na posição atual do mouse
         glEnd();
      }
   }
...
void pickPrimitive(int y)
{
   ...
   // função pickPrimitive para salvar a polilinha antes de mudar
   // Se existe uma polilinha sendo desenhada atualmente
   if(primitive == POLYLINE && currentPoline != nullptr)
   {
      // Adiciona a polilinha atual ao vetor de polilinhas
      polines.push_back(*currentPoline);
      // Libera a memória alocada para a polilinha atual
      delete currentPoline;
      // Zera o ponteiro da polilinha atual
...
void mouseControl(int button, int state, int x, int y)
{
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        else
        {
            ...
            else if(primitive == POLYLINE) //   Se a primitiva selecionada for uma polilinha
            {
                if (pointCount == 0) // Se é o primeiro ponto da polilinha
                {
                  // Cria uma nova polilinha com as coordenadas iniciais (x,y)
                  // e as propriedades atuais (cor RGB, preenchimento, tracejado)
                    currentPoline = new Poline(x, y, currentRed, currentGreen, currentBlue,
                                              currentFilled, currentDashed);
                    pointCount++; // Incrementa o contador de pontos
                    isDrawing = true; // Indica que começou o desenho
                }
                else // Se não for o primeiro ponto
                {
                    if(currentPoline != nullptr) // Verifica se existe uma polilinha atual
                    {
                        currentPoline->addPoint(x, y); // Adiciona um novo ponto à polilinha existente
                    }
                }
            }
